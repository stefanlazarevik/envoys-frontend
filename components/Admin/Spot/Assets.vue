<template>
  <div>
    ...
    <template v-if="assets.length">
      ...
      <!-- Start: data table -->
      <v-data-table :class="count > limit ? 'none-radius ' : ''" :headers="headlines" :items="assets" :page.sync="page" item-key="id" :server-items-length="length" :items-per-page="limit" hide-default-footer>
        ...
        <template v-slot:item.balance="{ item }">
          {{ $decimal.format(item.balance ? item.balance : 0) }} <b>{{ (item.symbol).toUpperCase() }}</b>
        </template>
      </v-data-table>
      <!-- End: data table -->
