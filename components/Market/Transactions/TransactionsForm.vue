<template>
  <v-card class="ma-2 pa-0" elevation="0" rounded="lg">
    ...
    <v-card-text>
      ...
      ></v-textarea>
      <v-btn @click="setRequest" block color="green darken-1 white--text" elevation="0">Set Transaction</v-btn>
    </v-card-text>
  </v-card>
</template>
  ...
  export default {
    ...
    methods: {
      setRequest() {
        ...
        if (!transaction.asset || !transaction.amount || !transaction.date || !transaction.area) {
